...
- Precompiled stylesheet at `@booga/vchat/styles.css` — zero-config adoption. A consumer with no Tailwind pipeline imports this one file and the chat UI renders styled; no preset, no content globs, no Tailwind build. Covers vChat's classes and the vUi / vForms classes it renders through.

### Changed

- `@booga/vui` and `@booga/vforms` dependencies raised to `^0.3.0`.
